...
public static class Guidance {
	...
	private static void ThrusterTick() {
		...
		force += Situation.mass * -2.0 * (pathNormal * pathLen / ttt / ttt - directNormal * directVel.Length() / ttt);
...
public static class Pilot {
	...
	private static void CalculateApproach() {
		...
		qInitialInverse = Quaternion.Inverse(Quaternion.CreateFromForwardUp(connector.WorldMatrix.Forward, referenceUp));
		...
		Navigation.AddWaypoint(newPos, newForward, newUp, APPROACH_SPEED, Waypoint.wpType.DOCKING);
		newPos = dock[0].stance.position + rotatedConnectorToCenter + ((DOCK_DISTANCE + connectorDistance) * dock[0].stance.forward);
		Navigation.AddWaypoint(newPos, newForward, newUp, MAX_SPEED, Waypoint.wpType.APPROACHING);
	...
	private static void SetEndStance(Dock dock) {
		...
		Navigation.AddWaypoint(undockPos, Situation.forwardVector, Situation.upVector, APPROACH_SPEED, Waypoint.wpType.UNDOCKING);
	}
...
public static class DockData {
	...
	private static void AddDock(bool stance) {
		Dock d = Dock.NewDock(RemoteControl.block.CenterOfMass, stance ? RemoteControl.block.WorldMatrix.Forward : Vector3D.Zero, stance ? RemoteControl.block.WorldMatrix.Up : Vector3D.Zero, Helper.FormatedWaypoint(stance, currentDockCount));
		currentDockCount++;
		docks.Add(d);
		docks.Sort();
		BalanceDisplays();
...
public static class Signal {
	public enum SignalType { DOCK, NAVIGATION, START };
...
public static class ConnectorControl {
	...
	public static IMyShipConnector GetConnector(Dock refDock) {
		if(Math.Abs(Vector3D.Dot(refDock.stance.forward, RemoteControl.block.WorldMatrix.Up)) < 0.5f) {
			foreach(IMyShipConnector connector in GridBlocks.shipConnectors) {
				if(Math.Abs(Vector3D.Dot(connector.WorldMatrix.Forward, RemoteControl.block.WorldMatrix.Up)) < 0.5f) {
					return connector;
				}
			}
		} else {
			foreach(IMyShipConnector connector in GridBlocks.shipConnectors) {
				if(Vector3D.Dot(connector.WorldMatrix.Forward, -refDock.stance.forward) > 0.5f) {
					return connector;
				}
			}
		}
...
public static class Pannels {
	...
	public enum ScreenAction {
		Prev, Next, Select, Add, Rem, AddStance
	};
